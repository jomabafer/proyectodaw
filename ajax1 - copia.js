...
function inicializarEventos(){
	...
	if(sesionIniciada==true){
		iniciarSesion("conexion.php");
	}else{
		cargarFormulario("formulario.php");
	}
...
function borrarHtml(div,pagina){
	//$(div).html("<a id='botonAnyadir' onclick=cargar('#divAnyadir','nuevaPublicacion.php') class='btn btn-primary'>Nueva Publicación</a><a id='botonAnyadir' onclick=cargar('#divAnyadir','publicarFoto.php') class='btn btn-primary'>Publicar Foto</a>");
	$(div).html("<a id='botonAnyadir' onclick=cargar('#divAnyadir','publicarFoto.php') class='btn btn-primary'>Publicar Foto</a>");
}
...
function validarDatos(data){
	...
	try{
		...
		var datos = JSON.parse(data); //Se decodifican los datos recibidos mediante JSON.
...
necesario invocar el método que llama al fichero listarAmistades (Posiblemente renombrado como detallesPerfil o algo así) y después ejecutar este código*/
		...
		//panelPrincipal.innerHTML = panelPrincipal.innerHTML + "<div id='divAnyadir'><a id='botonAnyadir' onclick=cargar('#divAnyadir','nuevaPublicacion.php') class='btn btn-primary'>Nueva Publicación</a><a id='botonAnyadir' onclick=cargar('#divAnyadir','publicarFoto.php') class='btn btn-primary'>Publicar foto</a></div>"; //Se añade un botón para desplegar mediante el método cargar un pequeño formulario que permite crear una publicación
		panelPrincipal.innerHTML = /*panelPrincipal.innerHTML + */" <div id='divAnyadir'><a id='botonAnyadir' onclick=cargar('#divAnyadir','publicarFoto.php') class='btn btn-primary'>Publicar foto</a></div>"; //Se añade un botón para desplegar mediante el método cargar un pequeño formulario que permite crear una publicación
		...
		if(datos.tipo == "admin"){ //Se muestran opciones de administración si el usuario conectado es de tipo admin.
			panelPrincipal.innerHTML = panelPrincipal.innerHTML + "<div id='administracion'></div>";
			cargar('#administracion','opcionesAdministracion.php');
			listarUsuarios(); 
...
function registrar(url){ 
	...
	if(datosValidos&clavesCoinciden){*/
		...
		conexionDatos.send('usuario='+JSON.stringify(usuarioCreado)+'&todo='+JSON.stringify(vectorDatos));
...
function publicar(){
	...
	conexionDatos.open("POST","nuevaPublicacionBackend.php", true);
	conexionDatos.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
...
function listarComentarios(usuario){
	$.ajax({
		...
		dataType: "html",
		contentType: "application/x-www-form-urlencoded",
		url: "notificacionesComentarios.php",
		data: "usuario=" + usuario,
		beforeSend: inicioEnvio,
